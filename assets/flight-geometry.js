...
export function flightProgress({width,height,scroll,heroBottom,gridY}) {
  const stripW=clamp(width*.24,300,380), anchor=heroBottom+stripW*.78;
...
export function tilePose(c,i,{width,scroll,count,drift,gridX,gridY,gridW,gridH,stripW,p,yBase}) {
  ...
  return {x:mix(x-(c.x+c.w/2),0,k),y:mix(y-(c.y-scroll+c.h/2),0,k)-Math.sin(k*Math.PI)*42,rotation:-11*(1-Math.min(k*1.35,1)),scale:mix(stripW/c.w,1,k)};
}
